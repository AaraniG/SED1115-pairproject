...
#Constant for PWM
PWM_PIN = Pin(2) #set to the GPIO Pin number -. GPIO2
...
while True:
    ...
    desired_pwm_value = float(input("Enter desired PWM Value (0-100%): ")) # Prompts user to enter desired PWM value as a percentage (0-100%)
    
    pwm.duty_u16(int((desired_pwm_value / 100)* 65535)) # Set the PWM duty cycle based on the desired value
    
    '''
    Calculates the corresponding duty cycle value for the PWM signal using the desired PWM value.
    Does this by converting the percentage to a 16-bit value within the range of 0 to 65535.
    '''
